...
#include "abb_irb120_commander/Num.h"
#include <sstream>
#include "iostream"

class MoveEnable{
  public:
    void cmdCallback(const abb_irb120_commander::Num::ConstPtr& msg);
};
...
// moveit_visual_tools::MoveItVisualToolsPtr visual_tools_;
  ...
  void MoveEnable::cmdCallback(const abb_irb120_commander::Num::ConstPtr& msg){
    ...
    moveit::planning_interface::MoveGroupInterface move_group("arm120");
    moveit::planning_interface::PlanningSceneInterface planning_scene_interface;
    const robot_state::JointModelGroup* joint_model_group =
        move_group.getCurrentState()->getJointModelGroup("arm120");

    move_group.setPoseReferenceFrame("base_iiwa");
    ...
    std::string tcp_link = move_group.getEndEffectorLink().c_str();
...
int main(int argc, char** argv)
{
  ...
  static const std::string PLANNING_GROUP = "arm120";
  moveit::planning_interface::MoveGroupInterface move_group(PLANNING_GROUP);

  MoveEnable mv;
  ROS_INFO("Esperando angulos articulares en topico robot_commander/cmd_joints...");
  ros::Subscriber sub = node_handle.subscribe("robot_commander/cmd_joints", 1000, &MoveEnable::cmdCallback, &mv);
